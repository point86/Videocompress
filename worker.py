...
class Worker(QObject):
    ...
    def operationRunner(self):
        ...
        self.emitLog.emit(INFO, "Converted files: %d - copied files: %d - errors: %d" % (self.converted, self.copied, self.fails))
        self.finished.emit()
    ...
    def convert_or_copy(self, input_file, output_dir):
        ...
        self.emitProgress.emit(round(100/self.totSize*self.processedSize), 0)
        self.emitLog.emit(INFO, "Processing: %s " % str(input_file))
        try:
